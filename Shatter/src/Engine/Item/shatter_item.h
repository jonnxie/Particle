...
struct PBRMaterial{
    ...
    glm::vec3 sheenColor;          // default: float3(0.0)
    float  sheenRoughness;      // default: 0.0

    glm::vec3 clearCoatNormal;     // default: float3(0.0, 0.0, 1.0)
    float  clearCoat;           // default: 1.0

    glm::vec3 anisotropyDirection; // default: float3(1.0, 0.0, 0.0)
    float  clearCoatRoughness;  // default: 0.0

    // only available when the shading model is subsurface
    glm::vec3 subsurfaceColor;     // default: float3(1.0)
    float  subsurfacePower;     // default: 12.234

    // only available when the shading model is specularGlossiness
    glm::vec3 specularColor;       // default: float3(0.0)
    float  glossiness;          // default: 0.0

    // not available when the shading model is unlit
    // must be set before calling prepareMaterial()
    glm::vec3 normal;              // default: float3(0.0, 0.0, 1.0)
    // only available when refraction is enabled
    float transmission;         // default: 1.0

    glm::vec3 absorption;          // default float3(0.0, 0.0, 0.0)
